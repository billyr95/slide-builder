...
def generate_ts(title_fit, subtitle_fit, image_fit):
    ...
    lines.append("  const oldValue = Math.max(24, Math.min(250, Math.round(rawFitted / 4) * 4))")
    lines.append("")
    lines.append("  // TITLE_SIZE_BIAS_PX is a manual, non-fitted bump (see its definition above) --")
    lines.append("  // applied after the fitted bucket/nudge math, before the final snap+clamp. The")
    lines.append("  // 250 ceiling matches the real editor slider's max (bumped from 160) -- none of")
    lines.append("  // the fitted bucket/nudge combinations get anywhere near it even with the bias,")
    lines.append("  // so this is headroom for intentionally large manual titles, not a bound this")
    lines.append("  // function actively pushes against today.")
    lines.append("  const snapped = Math.round((rawFitted + TITLE_SIZE_BIAS_PX) / 4) * 4")
    lines.append("  const newValue = Math.max(24, Math.min(250, snapped))")
    lines.append("")
